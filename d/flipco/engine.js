async function initHome(){
  try{
    ...
    if(b)b.innerHTML=brands.map(x=>{const p=ps.find(q=>q.brand===x);return `<a href="shop.html?brand=${encodeURIComponent(x)}"><span>${escapeHtml(x)}</span><b>↗</b></a>`}).join('');
    document.querySelectorAll('.category-grid a').forEach(card=>{const cat=(new URL(card.href,location.href)).searchParams.get('category');const p=ps.find(x=>x.category===cat);if(p?.image){card.style.setProperty('--cat-image',`url(\"${p.image}\")`);card.dataset.hasImage='true';}});
    ...
    if(hero&&slides.length){
      ...
      let index=0;
      const show=(n)=>{const all=[...hero.querySelectorAll('.hero-slide')]; if(!all.length)return; index=(n+all.length)%all.length; all.forEach((s,i)=>{s.classList.toggle('is-active',i===index);s.setAttribute('aria-hidden',String(i!==index))});dots?.querySelectorAll('button').forEach((d,i)=>d.classList.toggle('active',i===index));const counter=document.querySelector('.hero-index');if(counter)counter.textContent=`${String(index+1).padStart(2,'0')} — ${String(all.length).padStart(2,'0')}`;};
