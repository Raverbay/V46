async function initProduct(){
 ...
 <section class="v18-pdp-info">
   ...
   <p class="v18-tax">IVA inclusa</p><div class="z-pdp-rule"></div>
   <div class="z-pdp-stock">${stock?`DISPONIBILE · ${stock} ${stock===1?'PEZZO':'PEZZI'}`:'ESAURITO'}</div>
   <div class="z-size-head"><span>SELEZIONA LA TAGLIA</span><button type="button" id="sizeGuide">GUIDA TAGLIE</button></div>
   <div class="z-pdp-sizes">${(p.sizes||[]).map(s=>`<button type="button" class="pdp-size ${sizeStock(p,s)?'':'disabled'}" ${sizeStock(p,s)?'':'disabled'} data-size="${escapeHtml(s)}">${escapeHtml(s)}</button>`).join('')}</div>
   <button class="z-add-main v18-add-main" id="addBtn" disabled>SELEZIONA UNA TAGLIA</button>
   <div class="v18-delivery"><div><span>↗</span><p><strong>SPEDIZIONE</strong><br>€ 5,90 · gratuita da € 99</p></div><div><span>⌖</span><p><strong>RITIRO IN STORE</strong><br>Gratis · Via Italia 22, Cagliari</p></div></div>
   <div class="z-pdp-info-links"><details open><summary>DESCRIZIONE</summary><p>Selezione Flip&Co della stagione ${escapeHtml(p.season)}. ${escapeHtml(p.material)} · ${escapeHtml(p.fit)} · ${escapeHtml(p.color)}.</p></details><details><summary>DETTAGLI</summary><p>Brand: ${escapeHtml(p.brand)}<br>Categoria: ${escapeHtml(p.category)}<br>Disponibilità collegata allo stock reale del negozio.</p></details><details><summary>CONSEGNA & RITIRO</summary><p>Spedizione € 5,90, gratuita da € 99. Ritiro gratuito in Via Italia 22, Cagliari.</p></details></div>
   <div class="z-store-pickup"><span>FLIP&CO · CAGLIARI</span><strong>VUOI PROVARLO DAL VIVO?</strong><a target="_blank" rel="noopener" href="https://wa.me/393661087819?text=${encodeURIComponent('Ciao Flip&Co, vorrei provare '+p.brand+' '+p.name+' in store.')}">VERIFICA DISPONIBILITÀ IN STORE ↗</a></div>
 </section>`;
 ...
 const wish=root.querySelector('.v18-pdp-wish');wish.onclick=()=>{let a=[];try{a=JSON.parse(localStorage.getItem('flipco_wishlist')||'[]')}catch{}const has=a.includes(p.id);a=has?a.filter(x=>x!==p.id):[...a,p.id];localStorage.setItem('flipco_wishlist',JSON.stringify(a));wish.classList.toggle('saved',!has);wish.textContent=has?'♡':'♥'};
}
